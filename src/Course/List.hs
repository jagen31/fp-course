...
seqOptional = foldRight go $ Full Nil
  where go Empty _ = Empty
        go _ Empty = Empty
        go (Full x) (Full acc) = Full $ x :. acc
...
--
-- notReverse x ++ notReverse y cannot place any elements of y before the elements of x.  this means notReverse (y ++ x) has to
-- put all elements of y after the elements of x.  All changes which are not reversing must depend on the length of x and y.  But
-- x and y can be chosen differently for the same list.
notReverse ::
  List a
  -> List a
notReverse = reverse
