...
toList ::
  ListZipper a
  -> List a
toList (ListZipper c m j) = reverse c ++ listh [m] ++ j
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper c m j) = ListZipper c (f m) j
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper c m j) = MLZ $ go (break f c)
  where go (_, Nil) = Empty
        go (j', m':.c') = Full $ ListZipper c' m' (reverse j' ++ m:.j) 
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper c m j) = MLZ $ go (break f j)
  where go (_, Nil) = Empty
        go (c', m':.j') = Full $ ListZipper (reverse c' ++ m:.c) m' j'
...
moveLeftLoop (ListZipper Nil m j) = ListZipper j' m' Nil
  where m':.j' = reverse (m :. j)
moveLeftLoop (ListZipper (c:.cs) m j) = ListZipper cs c (m:.j)
...
moveRightLoop (ListZipper c m Nil) = ListZipper Nil m' c'
  where m':.c' = reverse (m:.c)
moveRightLoop (ListZipper c m (j:.js)) = ListZipper (m:.c) j js
...
moveLeft (ListZipper (c:.cs) m j) = isZ $ ListZipper cs c (m:.j)
...
moveRight (ListZipper c m (j:.js)) = isZ $ ListZipper (m:.c) j js
...
swapLeft (ListZipper (c:.cs) m j) = isZ $ ListZipper (m:.cs) c j
...
swapRight (ListZipper c m (j:.js)) = isZ $ ListZipper c j (m:.js)
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ m j) = ListZipper Nil m j
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper c m _) = ListZipper c m Nil
...
moveLeftN' n z@(ListZipper c _ _) = 
  if n < 0 then moveRightN' (negate n) z else go $ moveLeftN n z
  where 
    go (MLZ Empty) = Left $ length c
    go (MLZ (Full z')) = Right z'
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n z@(ListZipper _ _ j) =
  if n < 0 then moveLeftN' (negate n) z else go $ moveRightN n z
  where 
    go (MLZ Empty) = Left $ length j
    go (MLZ (Full z')) = Right z'
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z = moveRightN (n - index z) z
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper c m j) = let m':.c' = reverse (m:.j) in
  ListZipper (c' ++ c) m' Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper c m j) = let m':.j' = reverse (m:.c) in
  ListZipper Nil m' (j' ++ j)
...
deletePullLeft (ListZipper (c:.cs) _ j) = isZ $ ListZipper cs c j
...
deletePullRight (ListZipper c _ (j:.js)) = isZ $ ListZipper c j js
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper c m j) = ListZipper (m:.c) a j
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper c m j) = ListZipper c a (m:.j)
...
-- /Tip:/ Use `zipWith`
  (ListZipper c f j) <*> (ListZipper c' m j') =
    ListZipper (zipWith ($) c c') (f m) (zipWith ($) j j')
...
instance Extend ListZipper where
  f <<= z = ListZipper (unfoldr (go moveLeft) z) (f z) (unfoldr (go moveRight) z)
    where go m z = (\x -> (f x, x)) <$> (toOptional . m) z 
...
instance Traversable ListZipper where
  traverse f (ListZipper c m j) = 
    (lift3 ListZipper) (traverse f c) (f m) (traverse f j)
...
instance Traversable MaybeListZipper where
  ...
  traverse f (MLZ (Full z)) = isZ <$> traverse f z
