...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat l = fst $ runState (findM repeatK l) S.empty
...
isHappy ::
  Integer
  -> Bool
isHappy n = contains 1 $ firstRepeat (produce sumDigitsSquared n)
  where 
    sumDigitsSquared n' = toInteger $ sum $ ((P.^2) . digitToInt) <$> show' n'
