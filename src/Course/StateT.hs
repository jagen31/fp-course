...
instance Monad k => Monad (StateT s k) where
  ...
  k =<< st =
    StateT $ \s ->
      runStateT st s >>= (\(a, s') -> runStateT (k a) s')
...
repeatKT :: (Ord a, Monad k) => a -> StateT (S.Set a) k Bool
repeatKT a = getT >>= \s -> putT (S.insert a s) >> pure (not $ S.member a s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' l = eval' (filtering repeatKT l) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF l = evalT (filtering (\a -> 
  if a > 100 then StateT (\_ -> Empty) else repeatKT a) l) S.empty
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  OptionalT kf <*> OptionalT k = 
    OptionalT $ kf >>= onFull (\f -> k >>= onFull (\a -> pure (Full $ f a)))
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k b
  -- not good
  f =<< OptionalT k = OptionalT $ k 
    >>= onFull (\a -> runOptionalT (f a) 
      >>= onFull (\b -> pure $ Full b))
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    -> Logger l a
    -> Logger l b
  k =<< Logger l a = let (Logger l' b) = k a in
    Logger (l ++ l') b
...
-- well, it works
distinctG l = runOptionalT $ evalT (filtering go l) S.empty
  where
    go a =
      let big = a > 100
          log = (if big then log1 ("aborting > 100: " ++ show' a) else if even a 
            then log1 ("even number: " ++ show' a)
            else Logger Nil)
          next = if big then \_ -> StateT $ \_ -> OptionalT (Logger Nil Empty) else repeatKT in
        (StateT $ \s -> OptionalT $ log (Full (a, s))) >>= next
